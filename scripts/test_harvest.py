...
def fetch_fha_approved_condos():
    """Fetches list of active HUD / FHA approved condominium project names in WA."""
    approved_set = set()
    ...
    try:
        ...
        with urllib.request.urlopen(req, timeout=20) as resp:
            ...
            for m in re.finditer(r'<td[^>]*>(?:<font[^>]*>)?([A-Za-z0-9\s\.\,\'-]{3,80})(?:</font>)?</td>', html, re.IGNORECASE):
                val = m.group(1).strip()
                if len(val) >= 4 and not val.isdigit():
                    approved_set.add(slugify(val))
    except Exception as e:
        print(f"   ⚠️ FHA/HUD Approved List Notice: {e}")
    print(f"   [DIAGNOSTIC 🏛️] Verified {len(approved_set)} active FHA/HUD approved condo records.")
    return approved_set

def fetch_va_approved_condos():
    """Fetches list of active VA approved condominium project names in WA."""
    approved_set = set()
    print("   📡 Querying VA LGY Hub Approved Condominium Register (WA State)...")
    try:
        url = "https://lgy.va.gov/lgyhub/condo-report"
        req = urllib.request.Request(url, headers={"User-Agent": "MySeattleSearchBot/1.0", "Accept": "text/html,application/xhtml+xml"})
        with urllib.request.urlopen(req, timeout=20) as resp:
            html = resp.read().decode("utf-8", errors="ignore")
            for m in re.finditer(r'<td>([A-Za-z0-9\s\.\,\'-]{3,80})</td>', html, re.IGNORECASE):
                val = m.group(1).strip()
                if len(val) >= 4 and not val.isdigit():
                    approved_set.add(slugify(val))
    except Exception as e:
        print(f"   ⚠️ VA Approved List Notice: {e}")
    print(f"   [DIAGNOSTIC 🎖️] Verified {len(approved_set)} active VA approved condo records.")
    return approved_set
...
def harvest_condo_buildings():
    ...
    # Ingest Live HUD (FHA) and VA Approval Lists
    fha_approved_set = fetch_fha_approved_condos()
    va_approved_set = fetch_va_approved_condos()
    ...
    while True:
        ...
        for item in kc_socrata_res:
            ...
            raw_addr = item.get("addr_full") or item.get("situs_address") or item.get("address") or item.get("street_address")
    ...
    for b_idx, i in enumerate(range(0, len(major_keys), batch_size), start=1):
        ...
        if kc_gis_res and isinstance(kc_gis_res, dict):
            ...
            for feat in features:
                ...
                # Check Approval Roster Matches (Strict Boolean Validation)
                b_slug = slugify(b_name)
                is_fha = b_slug in fha_approved_set if fha_approved_set else False
                is_va = b_slug in va_approved_set if va_approved_set else False
    ...
    for city in city_boundaries:
        ...
        if sno_res and isinstance(sno_res, dict) and "features" in sno_res:
            ...
            for feat in features:
                ...
                raw_title = props.get("SITUSLINE1") or props.get("TAXPRNAME") or props.get("OWNERNAME") or f"Snohomish Condominium #{parcel_id}"
                b_name = clean_building_name(raw_title) or f"{city['name']} Ridge Condominiums"
...
def harvest_new_subdivisions():
    ...
    # 1. King County Recorded Plats Stream
    kc_plat_map = {}
    offset = 0
    limit = 5000
